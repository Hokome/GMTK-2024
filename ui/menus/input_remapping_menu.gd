...
func _ready():
	...
	action_list = $center_container/v_box_container2/v_box_container3
	#for c in action_list.get_children():
		#c.queue_free()
	
	for action in remappable_inputs:
		var remapper = input_remapper_scene.instantiate()
		var action_label: Label = remapper.get_node("action_label")
		var remap_button: Button = remapper.get_node("action_button")
